 ...
 * @author bc
 */
public class DefaultLookupDataLocator extends LookupDataLocator {

    protected String classFullName;

    public DefaultLookupDataLocator() {
    }

    public DefaultLookupDataLocator(String classFullName) {
    ...
    public Response validateCode(String code) {
        Session s = null;
        try {
            Long codeNumeric = new Long(-1);
            try {
                codeNumeric = Long.parseLong(code);
            } catch (Exception e) {
            }
            String sql = "FROM " + classFullName + " C " + "" +
                    "WHERE C.auditoria.activo=? AND C.id=?";
