        ...
        try {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                if (attributeName.startsWith("get") && methods[i].getParameterTypes().length == 0
                        ...
                        || methods[i].getReturnType().isEnum()//Nelson Moncada
                        || ValueObject.class.isAssignableFrom(methods[i].getReturnType()))) {
...
//        else
        ...
        } else if (mode == Consts.INSERT || mode == Consts.EDIT) // if the column index is related to an attribute not mapped with a grid column,
